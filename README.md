...
After cloning this project, the next step is to configure the project. This involves setting environment variables and secrets in AI Workbench.
...
After setting the appropriate environment variables you must set up secrets to access models or NVCF. 

Keep in mind, you must set the `HUGGING_FACE_HUB_TOKEN` secret to a valid Hugging Face API Token. You must have access to the Llama2 model in Hugging Face by requesting it here: [https://huggingface.co/meta-llama/Llama-2-7b-chat-hf](https://huggingface.co/meta-llama/Llama-2-7b-chat-hf).
...
You may be prompted to supply the following:  
...
On Desktop, simply run the `rag-server` application.
...
You're all set! Now just use the chat app and chat with your data. Select the `Use knowledge base` checkbox when you want to have your data constrain and inform the response.
...
Configuration is primarily done via Environment Variables, which can be easily set throught "Environment" tab for the project in AI Workbench.
